...
/* =========================
CORS
Allow the deployed AlphaEdge frontend to call this backend.
No frontend/index.html changes are required.
========================= */
...
try {
await pool.query(
  `INSERT INTO market_snapshots(symbol, ltp, close, change_pct)
   VALUES($1,$2,$3,$4)`,
...
const result = await pool.query(
  `SELECT ltp, close, change_pct, captured_at
...
/* =========================
EQUITY MARKET DATA PROXY
Yahoo Finance is called from the backend so the frontend is not blocked by
browser CORS restrictions. The frontend only talks to this AlphaEdge API.
========================= */
async function fetchYahooChart(symbol, range, interval) {
  const urls = [
    `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?range=${encodeURIComponent(range)}&interval=${encodeURIComponent(interval)}&events=div%2Csplits`,
    `https://query2.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?range=${encodeURIComponent(range)}&interval=${encodeURIComponent(interval)}&events=div%2Csplits`
  ];
  ...
  throw lastError || new Error('Yahoo market data unavailable');
}

function validEquityYahooSymbol(value) {
  return /^[A-Za-z0-9._-]{1,32}$/.test(String(value || ''));
}

app.get('/api/equity/chart', async (req, res) => {
  const symbol = String(req.query.symbol || '').trim().toUpperCase();
  const range = String(req.query.range || '5d').trim();
  const interval = String(req.query.interval || '1d').trim();

  if (!validEquityYahooSymbol(symbol)) {
    return res.status(400).json({ success: false, message: 'Invalid equity symbol' });
  }
  if (!['5d', '1y', '5y'].includes(range) || !['1d', '1mo'].includes(interval)) {
    return res.status(400).json({ success: false, message: 'Invalid chart range or interval' });
  }
  ...
  try {
    ...
    console.error(`Equity chart failed for ${symbol}:`, err.message);
    res.status(502).json({ success: false, message: 'Unable to load equity market data' });
  }
});
...
const result = await pool.query(
  `UPDATE clients
   ...
   RETURNING client_id,name,phone,email`,
  vals
);
...
const result = await pool.query(
  `INSERT INTO payment_confirmations
   (client_id,course,amount,utr,slip_name,slip_data)
   VALUES($1,$2,999,$3,$4,$5)
