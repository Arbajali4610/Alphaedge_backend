...
function initDatabase() {
  try {
    ...
    // Older AlphaEdge databases used `mobile`; keep it populated too.
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message:
            'Mobile number and email are required'
        });
      }
      ...
      // Store an empty hash for compatibility with older SQLite databases
      // whose password_hash column was created as NOT NULL. No password is
      // accepted or required by the passwordless client flow.
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!/^AE\d{6}$/.test(clientId)) {
        return res.status(400).json({
          success: false,
          message: 'Enter a valid Client ID, e.g. AE100001.'
        });
      }

      const result = await pool.query(
        `SELECT id, client_id, name, phone, email, status
         FROM clients
         WHERE client_id = $1
         LIMIT 1`,
        [clientId]
